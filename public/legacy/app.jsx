/* ============================================================
   app.jsx — root: state, theme, tab nav, layout
   ============================================================ */
...
function App() {
  ...
  const [theme, setTheme] = useStateApp(() => localStorage.getItem("qpa-theme") || "dark");
  const [language, setLanguage] = useStateApp(() => localStorage.getItem("qpa-language") || "tr");
  const [holdings, setHoldings] = useStateApp(() =>
    Object.entries(DEFAULT_LOTS).map(([t, lots]) => ({ t, lots })));
  const [dateRange, setDateRange] = useStateApp("2Y");
  const [profile, setProfile] = useStateApp("balanced");
  const [assumptions, setAssumptions] = useStateApp({ rf: 0.043, horizon: 5, paths: 2000 });
  const [tab, setTab] = useStateApp("overview");
  const [apiStatus, setApiStatus] = useStateApp({
    checked: false,
    ok: false,
    hasFinnhubKey: false,
    message: "Checking proxy",
  });
  const [rateLimitWarning, setRateLimitWarning] = useStateApp(false);
  const [historyBySymbol, setHistoryBySymbol] = useStateApp({});
  const [quoteBySymbol, setQuoteBySymbol] = useStateApp({});
  const [profileBySymbol, setProfileBySymbol] = useStateApp({});
  const [marketDataStatus, setMarketDataStatus] = useStateApp({
    status: "idle",
    message: "Mock price model",
    loaded: 0,
    requested: 0,
  });
  const [referenceDataStatus, setReferenceDataStatus] = useStateApp({
    status: "idle",
    quoteLoaded: 0,
    profileLoaded: 0,
    requested: 0,
  ...
  const p = useMemoApp(() => dataAdapter.buildPortfolio(holdings, {
