/* ============================================================
   charts.jsx — lightweight theme-aware SVG charts
   All colors pull from CSS vars so they track dark/light.
   ============================================================ */
...
function GrowthChart({ series, height = 250 }) {
...
function HBars({ data, valueFmt = v => (v * 100).toFixed(1) + "%", height }) {
  // data:[{label,value,color}]
...
function VBars({ data, height = 150, color = "var(--accent)", baseline = 0, fmt }) {
...
function MiniLine({ data, height = 150, color = "var(--accent-2)", fmt, band }) {
  const W = 760, H = height, padB = 18, padT = 8, padL = 40, padR = 8;
...
// ---------- CORRELATION HEATMAP ----------
function Heatmap({ tickers, corrFn }) {
  const [hover, setHover] = useState(null);
...
// ---------- MONTE CARLO FAN ----------
function FanChart({ sim, startValue, years, height = 280 }) {
...
function Histogram({ values, startValue, bins = 28, height = 180 }) {
...
Object.assign(window, { GrowthChart, Donut, HBars, VBars, MiniLine, Heatmap, FanChart, Histogram });
