/* ============================================================
   views-analysis.jsx — Analysis · Optimization · Simulation · Company · Data
   ============================================================ */
...
/* ---------------- ANALYSIS ---------------- */
function AnalysisTab({ p, language = "tr" }) {
...
/* ---------------- OPTIMIZATION ---------------- */
function OptimizationTab({ p, language = "tr" }) {
  const copy = moduleCopy(language);
...
/* ---------------- SIMULATION ---------------- */
function SimulationTab({ p, assumptions, language = "tr" }) {
  const copy = moduleCopy(language);
...
/* ---------------- COMPANY DATA ---------------- */
function CompanyTab({ p, referenceDataStatus, apiStatus, language = "tr" }) {
  const copy = moduleCopy(language);
  ...
  useEffectVA(function() {
    ...
    var apiBase = (window.DATA_SOURCES && window.DATA_SOURCES.real && window.DATA_SOURCES.real.baseUrl) || "http://127.0.0.1:8787";
...
/* ---------------- RAW DATA ---------------- */
function DataTab({ p, dateRange, apiStatus, marketDataStatus, referenceDataStatus, language = "tr" }) {
  const copy = moduleCopy(language);
...
Object.assign(window, { AnalysisTab, OptimizationTab, SimulationTab, CompanyTab, DataTab });
